...
public class ServerStopUpdateHandler {
	...
	public static void init() {
		...
		try {
            URL url = new URL("https://coolsimulations.net/mcmods/serverstop-fabric/versionchecker114.txt");
		...
		if(latestVersion != null) {
			...
			if(!latestVersion.equals(Reference.VERSION) && !latestVersion.equals("ended")) {
				...
				updateInfo = new TranslatableText(ServerStopEventHandler.getTranslations("sp.update.display1"), new Object[] {ss, version});
				updateInfo.getStyle().setColor(Formatting.YELLOW);
