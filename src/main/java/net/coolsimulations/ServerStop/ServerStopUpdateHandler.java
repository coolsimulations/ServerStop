...
public class ServerStopUpdateHandler {
	...
	public static void init() {
		...
		try {
			URL url = new URL("https://coolsimulations.net/mcmods/serverstop/versionchecker114.txt");
		...
		try {
			URL url = new URL("https://coolsimulations.net/mcmods/serverstop/updateinfo114.txt");
			Scanner s = new Scanner(url.openStream());
		...
		if(latestVersion != null) {
			...
			if(!latestVersion.equals(Reference.VERSION) && !latestVersion.equals("ended")) {
				...
				if(latestVersionInfo != null) {
					...
					updateVersionInfo.getStyle().setHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT, new StringTextComponent(ServerStopEventHandler.getTranslations("serverstop.update.display2"))));
